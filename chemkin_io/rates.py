"""
Writes a plog string
"""

def write_plog(reaction, params_dct, err_dct):
    ...
    # Build the top of the string with the fitting errors
    ...
    for key, val in err_dct.items():
        err_str = 'SumSqErr = {0}, MaxAvgErr = {1}, MeanAvgErr = {2}'.format(
            str(val[0]), str(val[1]), str(val[2]))
        p_str = '! {0}: {1}'.format(str(key), err_str)
